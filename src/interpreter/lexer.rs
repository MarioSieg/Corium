                    ...
                    match args.len().cmp(&meta.explicit_arguments.len()) {
                        ...
                        Ordering::Equal => {
                            for (i, ex_arg) in meta.explicit_arguments.iter().enumerate() {
                                ...
                                if !eval_param(&mut in2, &ex_arg.accepted_value_types[..]) {
                                    return false;
                                }
                            }
                            true
...
fn eval_arg_f32<'a>(in_: &mut LexIn<'a>) -> bool {
    if let Ok(x) = f32::from_str(in_.line) {
        ...
        in_.errors.push(format!(
            "Invalid decimal literal for f32: {:?}!",
            in_.backup
        ));
        false
    }
...
fn eval_arg_i32<'a>(in_: &mut LexIn<'a>) -> bool {
    if first_str_rem(in_.line, tok::LITERAL_PREFIX_HEX) {
        if let Ok(x) = i32::from_str_radix(in_.line, 6) {
            ...
            in_.errors.push(format!(
                "Invalid hexadecimal literal for i32: {:?}!",
                in_.backup
            ));
            false
        }
        ...
        if let Ok(x) = i32::from_str_radix(in_.line, 2) {
            ...
            in_.errors
                .push(format!("Invalid binary literal for i32: {:?}!", in_.backup));
            false
        }
        ...
        if let Ok(x) = i32::from_str_radix(in_.line, 8) {
            ...
            in_.errors
                .push(format!("Invalid octal literal for i32: {:?}!", in_.backup));
            false
        }
        ...
        in_.errors.push(format!(
            "Invalid decimal literal for i32: {:?}!",
            in_.backup
        ));
        false
    }
...
fn eval_arg_ipc<'a>(in_: &mut LexIn<'a>) -> bool {
    if let Ok(x) = i32::from_str_radix(in_.line, 16) {
        if x >= 0 && x < IntrinsicID::COUNT as i32 {
            in_.stream
                .with_intrin_id(IntrinsicID::from(Signal::from(x)));
            true
        } else {
            in_.errors.push(format!(
        ...
        in_.errors.push(format!(
            "Invalid hexadecimal literal for ipc: {:?}!",
            in_.backup
        ));
        false
    }
...
fn eval_param<'a>(in_: &mut LexIn<'a>, targets: &[ArgumentLiteralType]) -> bool {
    ...
    let mut eval =
        |f: &mut dyn FnMut(&mut LexIn<'a>) -> bool, target: &ArgumentLiteralType| -> bool {
            let found = {
                let mut found = false;
                'se: for tar in targets {
                    if std::mem::discriminant(tar) == std::mem::discriminant(target) {
                        found = true;
                        break 'se;
                    }
                }
            ...
            if found {
                ...
                error += &format!(
                    " <- Requires {:?} for i32, {:?} for f32, {:?} for pin, {:?} for ipc",
