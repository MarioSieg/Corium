...
impl OpCode {
    pub fn category(&self) -> OpCodeCategory {
        if (*self as usize) < Self::Push as usize {
            OpCodeCategory::Control
        } else if (*self as usize) > Self::NoOp as usize && (*self as usize) < Self::Jump as usize {
            OpCodeCategory::Memory
        } else if (*self as usize) > Self::CastF32toI32 as usize
            && (*self as usize) < Self::I32Addition as usize
        {
            OpCodeCategory::Branching
        } else if (*self as usize) > Self::JumpIfLessEquals as usize
            && (*self as usize) < Self::I32Vector4Addition as usize
        {
            OpCodeCategory::Arithmetics
        } else {
            OpCodeCategory::VectorArithmetics
        }
    }
}
...
impl fmt::Debug for Section {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", lexemes::SECTIONS[*self as usize])
    }
}
