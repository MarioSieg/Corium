...
pub fn fma8(x: &mut [f32; 8], y: &[f32; 8], z: &[f32; 8]) {
    unsafe {
        ...
        _mm_storeu_ps(
            x.offset(4),
            _mm_fmadd_ps(
                _mm_loadu_ps(x.offset(4)),
                _mm_loadu_ps(y.offset(4)),
                _mm_loadu_ps(z.offset(4)),
            ),
        );
    }
}
...
pub fn fma16(x: &mut [f32; 16], y: &[f32; 16], z: &[f32; 16]) {
    unsafe {
        ...
        _mm_storeu_ps(
            x.offset(8),
            _mm_fmadd_ps(
                _mm_loadu_ps(x.offset(8)),
                _mm_loadu_ps(y.offset(8)),
                _mm_loadu_ps(z.offset(8)),
            ),
        );
        _mm_storeu_ps(
            x.offset(12),
            _mm_fmadd_ps(
                _mm_loadu_ps(x.offset(12)),
                _mm_loadu_ps(y.offset(12)),
                _mm_loadu_ps(z.offset(12)),
            ),
        );
    }
}
...
mod tests {
    ...
    fn add4() {
        ...
        fallback::f32::add4(&mut x_fallback, &y);
    ...
    fn add8() {
        ...
        fallback::f32::add8(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn add16() {
        ...
        fallback::f32::add16(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn sub4() {
        ...
        fallback::f32::sub4(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn sub8() {
        ...
        fallback::f32::sub8(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn sub16() {
        ...
        fallback::f32::sub16(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn mul4() {
        ...
        fallback::f32::mul4(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn mul8() {
        ...
        fallback::f32::mul8(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn div4() {
        ...
        fallback::f32::div4(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn div8() {
        ...
        fallback::f32::div8(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
    ...
    fn div16() {
        ...
        fallback::f32::div16(&mut x_fallback, &y);
        for (x, y) in x_simd.iter().zip(x_fallback.iter()) {
            assert_eq!(x, y);
        }
    }
