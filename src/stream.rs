...
impl BytecodeStream {
    ...
    pub fn with_int(&mut self, x: i32) -> &mut Self {
        self.0
            .push((RecordUnion::from_int(x), RecordUnionDiscriminator::Int));
        self
    }
    ...
    pub fn with_float(&mut self, x: f32) -> &mut Self {
        self.0
            .push((RecordUnion::from_float(x), RecordUnionDiscriminator::Float));
        self
    }
    ...
    /// Validates the input bytecode stream and converts
    /// this to a bytecode chunk, which can be injected into
    /// a vm executor.
    pub fn to_bytecode_chunk(self) -> Result<BytecodeChunk, Vec<&'static str>> {
        if let Err(errors) = validator::validate_bytecode(&self) {
            Err(errors)
        } else {
            ...
            Ok(BytecodeChunk::with_buf(buf.into_boxed_slice()))
        }
    }

    pub fn write_to_disk_as_txt(&self, file: &path::Path) -> Result<(), io::Error> {
        interpreter::write_to_file(file, self)
    }
}
