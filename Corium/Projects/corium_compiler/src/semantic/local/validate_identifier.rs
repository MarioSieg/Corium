...
pub fn validate(errors: &mut ErrorList, input: &GlobalSymbolTable, local: &LocalSymbolTable) {
    for bucket in local.values() {
        if let Some(expr) = bucket.extract_expression() {
            validate_identifier::validate_identifiers(errors, expr, input, Some(local));
        }
    }
}
...
mod tests {
    ...
    fn valid() {
        ...
        let immutable_variable = GlobalStatement::ImmutableVariable(ImmutableVariable {
            ...
            value: Expression::Identifier(Identifier::new("ll")),
            type_hint: None,
        });
        ...
        let global_table = crate::semantic::global::symbol_table::populate(&mut errors, items);
        let local_smt = MutableVariable {
            name: Identifier::new("noel"),
            value: Expression::Identifier(Identifier::new("fy")),
            type_hint: None,
        };
        let ident = Identifier::new("ymm");
        let local_table = LocalSymbolTable::from([(&ident, Bucket::MutableVariable(&local_smt))]);
        validate(&mut errors, &global_table, &local_table);
        assert!(errors.is_empty());
        assert_eq!(global_table.len(), 4);
    }
    ...
    fn valid_parameter() {
        ...
        let mut errors = ErrorList::new();
        let global_table = crate::semantic::global::symbol_table::populate(&mut errors, items);
        let ident = Identifier::new("noel");
        let local_smt = MutableVariable {
            name: ident.clone(),
            value: Expression::Literal(Literal::Int(3)),
            type_hint: None,
        };
        let ident2 = Identifier::new("ymm");
        let local_smt2 = Parameter {
            name: ident2.clone(),
            value: Some(Expression::Identifier(Identifier::new("noel"))),
            type_hint: Identifier::new("int"),
        };
        let local_table = LocalSymbolTable::from([
            (&ident, Bucket::MutableVariable(&local_smt)),
            (&ident2, Bucket::Parameter(&local_smt2)),
        ]);
        validate(&mut errors, &global_table, &local_table);
        assert!(errors.is_empty());
        assert_eq!(global_table.len(), 4);
    ...
    fn invalid() {
        ...
        let local_smt = MutableVariable {
            name: Identifier::new("noel"),
            value: Expression::Identifier(Identifier::new("zmm")),
            type_hint: None,
        };
        let ident = Identifier::new("ymm");
        let local_table = LocalSymbolTable::from([(&ident, Bucket::MutableVariable(&local_smt))]);
    ...
    fn invalid_parameter() {
        ...
        let local_smt2 = Parameter {
            name: ident2.clone(),
            value: Some(Expression::Identifier(Identifier::new("noel323"))),
            type_hint: Identifier::new("int"),
        };
        let local_table = LocalSymbolTable::from([
            (&ident, Bucket::MutableVariable(&local_smt)),
