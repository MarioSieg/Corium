...
impl<'a> LocalState<'a> {
    ...
    pub fn insert_immutable_variable(
        &mut self,
        statement: &LocalStatement,
        variable: &'a ImmutableVariable<'a>,
    ) -> Result<(), ErrorList> {
        if let Some(existing) = self
            .table
    ...
    pub fn definition_error(&self, previous: &Record, current: &LocalStatement) -> ErrorList {
        ...
        let smt_ident = current.code_identifier().full;
        let fun_name = self.signature().name.to_string();
    ...
    #[cold]
    pub fn global_definition_error(
        &self,
        previous: &Record,
    ...
    #[cold]
    pub fn unexpected_return_error(&self, statement: &ReturnStatement) -> ErrorList {
        let fun_name = self.signature().name.to_string();
        let smt = format!("{}", statement);
        let message = format!(
    ...
    pub fn missing_return_expr_error(&self, required_type: &Identifier) -> ErrorList {
        let fun_name = self.signature().name.to_string();
        let required_type = required_type.full;
        let message = format!(
