...
namespace Nominax {
	auto perform_bytecode_validation_for_instruction(const Instruction instr_, const std::span<DynamicSignal> args_) -> validation_result_error {
		...
		const std::uint8_t required_arg_count = imm_arg_counts[instr_idx];
		...
		const std::array<instruction_imm_arg_type, max_imm_args>& type_table = imm_arg_types[instr_idx];
		...
		for (std::size_t i = 0; i < args_.size(); ++i) [[likely]] {
			...
			const instruction_imm_arg_type required_type = type_table[i];

			// true if the data types are equal, else false
			bool correct_type;
			
			switch (required_type) {
				case instruction_imm_arg_type::i64: 
					correct_type = arg.Contains<std::int64_t>();
				break;
				case instruction_imm_arg_type::u64:
				case instruction_imm_arg_type::rel64:
				case instruction_imm_arg_type::abs64:
					correct_type = arg.Contains<std::uint64_t>();
				break;
				case instruction_imm_arg_type::intrin64:
					correct_type = arg.Contains<SystemIntrinsicID>() || arg.Contains<CustomIntrinsicID>();
				break;
				case instruction_imm_arg_type::f64: 
					correct_type = arg.Contains<double>();
				break;
				case instruction_imm_arg_type::i64_u64:
					correct_type = arg.Contains<std::int64_t>() || arg.Contains<std::uint64_t>();
				break;
				case instruction_imm_arg_type::i64_u64_f64: 
