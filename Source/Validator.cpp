...
namespace Nominax
{
	auto perform_bytecode_validation_for_instruction(const Instruction              instr_,
	                                                 const std::span<DynamicSignal> args_) -> validation_result_error
	{
		const auto         instr_idx          = static_cast<std::size_t>(instr_);
		const std::uint8_t required_arg_count = INSTRUCTION_IMMEDIATE_ARGUMENT_COUNTS[instr_idx];

		// check if we submitted not enough arguments:
		if (args_.size() < required_arg_count)
		[[unlikely]]
		{
		...
		[[likely]]
		{
			...
			const InstructionImmediateArgumentType required_type = type_table[i];
