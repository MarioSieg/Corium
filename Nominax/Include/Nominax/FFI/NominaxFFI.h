...
/*
 * This header provides the NNI (Nominax Native Interface),
 * which is used to provide the FFI (Foreign Function Interface) for Corium.
 */

#ifndef NOMINAX_FFI_H
#define NOMINAX_FFI_H
...
extern "C" {
	...
	#	define NOX_RESTRICT restrict
	#endif

	#ifdef _MSC_VER
	#	define NOX_CALLCONV __cdecl
	#	define NOX_DYEXPORT __declspec(dllexport)
	#else
	...
	#	define NOX_DYEXPORT __attribute__((visibility("default")))
		...
		extern "C++"
		{
			namespace Nominax::FFI
			{
				...
				constexpr auto RecordToFFIRecord(const NOX_Record x) noexcept -> ::Nominax::Foundation::Record
				{
					#ifdef NOX_FFI_NO_BIT
						return *reinterpret_cast<::Nominax::Foundation::Record*>(&x);
					#else
	...
	#define NOX_DLL_EXPORT NOX_DYEXPORT

	/* Nominax FFI calling convention. */
	...
	#define NOX_SP_LDA_DEREF(spRel, moff) (*((spRel)+(moff)))
