...
{
	/// <summary>
	/// VM cluster bundles multiple VM execution engines.
	/// </summary>
	struct [[nodiscard]] Cluster final
	{
		explicit Cluster(std::uint64_t concurrency);
		Cluster(const Cluster& other) = delete;
		Cluster(Cluster&& other) noexcept = default;
		auto operator =(const Cluster& other) -> Cluster& = delete;
