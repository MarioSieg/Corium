...
#include "../../Foundation/VectorLib.hpp"
#include "../../Foundation/IEEE754/Compare.hpp"
#include "../../Foundation/Algorithm/BitRotation.hpp"
#include "../../Foundation/CPU/Trap.hpp"
...
namespace Nominax::Core::VM
{
	...
	{
		static constexpr std::array<const void* const, Foundation::Algorithm::ToUnderlying(Instruction::Count_)> JUMP_TABLE
		{
			&&$int$,
			&&$syscall$,
			&&$intrin$,
	...
	$int$:
		NOX_COLD_LABEL;
		{
            ...
            (*interruptHandlerHook)(std::move(interrupt));
            goto _terminate_;
		}
		goto
		PEEK_DISPATCH();
	...
	_terminate_:
        ...
        output->PreStamp = pre;
        output->PostStamp = post;
        output->ExecutionTime = post - pre;
        output->IPDifference = ip - *input->Image;
        output->SPDifference = sp - *input->Stack;
        output->BPDifference = ip - bp;
        return true;
	}
}
