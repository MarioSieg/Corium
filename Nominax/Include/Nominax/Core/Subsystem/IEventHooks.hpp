...
namespace Nominax::Core::Subsystem
{
	...
	constexpr auto IEventHooks::Subscribe(const HookFlags hook) const & noexcept -> void
	{
		this->HookFlags_ |= Foundation::Algorithm::ToUnderlying(hook);
	}

	constexpr auto IEventHooks::Unsubscribe(const HookFlags hook) const & noexcept -> void
	{
		this->HookFlags_ &= ~Foundation::Algorithm::ToUnderlying(hook);
	}

	constexpr auto IEventHooks::ToggleSubscription(const HookFlags hook) const & noexcept -> void
	{
		this->HookFlags_ ^= Foundation::Algorithm::ToUnderlying(hook);
	}

	constexpr auto IEventHooks::SubscribedHooks() const & noexcept -> HookFlags
	{
		return Foundation::Algorithm::FromUnderlying<HookFlags>(this->HookFlags_);
	}
	...
	constexpr auto IEventHooks::SetSubscriptions(const HookFlags collection) const & noexcept -> HookFlags
	{
