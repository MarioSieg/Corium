...
#define VALIDATE_ONLINE_BOOT_STATE() NOX_PANIC_ASSERT_TRUE(this->IsOnline(), "Environment is offline!")
		...
		/// <returns></returns>
		[[maybe_unused]]
		static auto PrintByteCodeErrorSector(const std::size_t idx, const ByteCode::Stream& appCode, const ByteCode::ValidationResultCode code)
		{
			...
			for (std::size_t i{ idx }; i < idx + 8 && i < appCode.Size(); ++i)
			{
				...
				{
					Common::Print(Common::TextColor::Green, "\n{:#018X}: ", i);
					Common::Print(Common::TextColor::BrightBlue, "{}", appCode[i].Value.R64.AsU64);
				}
				else
				{
					Common::Print(Common::TextColor::Magenta, " {}", appCode[i].Value.R64.AsU64);
				}

				if (isInstructionFault && i == idx)
				{
					Common::Print(Common::LogLevel::Error, " {} ->", code);
				}
			}
		...
		/// <returns></returns>
		[[noreturn]]
		static auto TriggerByteCodeStreamValidationPanic(const ByteCode::ValidationResultCode result, [[maybe_unused]] const ByteCode::Stream& appCode)
		{
			// Print error message:
		...
		auto Environment::Execute(ByteCode::Stream&& appCode) -> std::pair<ReactorShutdownReason, const ReactorState&>
		{
			...
			// Info
			Common::Print(Common::LogLevel::Warning, "Executing...\n");
			std::cout.flush();
			...
			// Print exec info:
			const auto level{ result.first == ReactorShutdownReason::Success ? Common::LogLevel::Success : Common::LogLevel::Error };
			Common::Print(level, "Execution #{} done! Runtime {:.04}\n", this->Context_->ExecutionTimeHistory.size(), std::chrono::duration_cast<std::chrono::duration<F64, std::ratio<1>>>(micros));
			std::cout.flush();
