...
namespace Nominax
{
	...
	namespace Core
	{
		VerboseReactorDescriptor BasicReactorDescriptor::BuildDetailed() const
		{
			return
			{
		...
		ReactorValidationResult BasicReactorDescriptor::Validate() const
		{
		...
		/// </summary>
		/// <returns></returns>
		static System::Snapshot InitSysInfo()
		{
			Common::Print('\n');
		...
		static System::CpuFeatureDetector InitCpuFeatures()
		{
			Common::Print('\n');
		...
		/// <returns></returns>
		template <typename T>
		static inline void PrintTypeInfo(const std::string_view name)
		{
		...
		/// <returns></returns>
		static void PrintTypeInfoTable()
		{
...
#define VALIDATE_ONLINE_BOOT_STATE() NOX_PANIC_ASSERT_TRUE(this->IsOnline(), "Environment is offline!")
		...
		/// <returns></returns>
		static inline std::size_t MapStackSize(const std::size_t sizeInBytes)
		{
		...
		/// <returns></returns>
		static inline std::size_t ComputePoolSize
		(
		...
		/// <returns></returns>
		static inline U8* operator*(const std::unique_ptr<U8[]>& ptr)
		{
			return ptr.get();
		}
		...
		/// <param name="desiredSize"></param>
		/// <returns></returns>
		static constexpr std::size_t ClampBootPoolSize(const std::size_t desiredSize)
		{
		...
		/// <returns></returns>
		[[maybe_unused]]
		static auto PrintByteCodeErrorSector(const std::size_t idx, const ByteCode::Stream& appCode,
		...
		/// </summary>
		[[nodiscard]]
		NOX_ALLOC_SIZE(1) static inline U8* AllocatePool(const std::size_t size, const std::string_view poolId)
		{
		...
		/// <returns></returns>
		[[noreturn]]
		static auto TriggerByteCodeStreamValidationPanic(const ByteCode::ValidationResultCode result,
		...
		/// <returns></returns>
		[[nodiscard]]
		static constexpr F64 ComputeMemoryPercent(const std::size_t used, const std::size_t max)
		{
		...
		/// <returns></returns>
		[[nodiscard]]
		static std::pair<std::ptrdiff_t, F64> QueryMemoryResourceUsage
		(
		...
		void Environment::ContextDeleter::operator()(Context* const kernel) const
		{
			delete kernel;
		}

		bool Environment::OnPreBootHook()
		{
			return true;
		}

		bool Environment::OnPostBootHook()
		{
			return true;
		}

		bool Environment::OnPreExecutionHook([[maybe_unused]] const ByteCode::AppCodeBundle& appCodeBundle)
		{
			return true;
		}

		bool Environment::OnPostExecutionHook()
		{
			return true;
		}

		bool Environment::OnPreShutdownHook()
		{
			return true;
		}

		bool Environment::OnPostShutdownHook()
		{
			return true;
		}
		...
		void Environment::Boot(const EnvironmentDescriptor& descriptor)
		{
			if (this->Context_)
			{
				[[unlikely]]
		...
		std::pair<ReactorShutdownReason, const ReactorState&> Environment::Execute(ByteCode::Stream&& appCode)
		{
		...
		bool Environment::IsOnline() const
		{
			return this->Context_ != nullptr;
		}

		const void* Environment::GetKernel() const
		{
			return this->Context_.get();
		}

		std::chrono::high_resolution_clock::time_point Environment::GetBootStamp() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->BootStamp;
		}

		std::chrono::milliseconds Environment::GetBootTime() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->BootTime;
		}

		const System::Snapshot& Environment::GetSystemSnapshot() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->SysInfoSnapshot;
		}

		const System::CpuFeatureDetector& Environment::GetProcessorFeatureSnapshot() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->CpuFeatures;
		}

		const std::pmr::string& Environment::GetAppName() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->AppName;
		}

		std::size_t Environment::GetMonotonicSystemPoolSize() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->SystemPoolSize;
		}

		std::size_t Environment::GetExecutionCount() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->ExecutionTimeHistory.size();
		}

		const std::pmr::vector<std::chrono::duration<F64, std::micro>>& Environment::GetExecutionTimeHistory() const
		{
			VALIDATE_ONLINE_BOOT_STATE();
			return this->Context_->ExecutionTimeHistory;
		}

		bool ComputeInstructionMapBinding(std::span<const ByteCode::Signal::Discriminator> input,
	...
	/// <returns></returns>
		...
		void WriteHardFaultReport(const std::string_view message)
		{
			WriteHardFaultReport(nullptr, nullptr, nullptr, 0, 0, message, 0, 0);
		}

		void DefaultInterruptRoutine(InterruptAccumulator)
		{
		}

		InterruptRoutine* GetDefaultInterruptRoutine()
		{
			return &DefaultInterruptRoutine;
		}

		bool Object::ShallowCopyObjectBlockToBuffer(const std::span<Record> buffer) const
		{
		...
		void Object::ShallowCopyObjectBlockToBuffer(std::vector<Record>& buffer) const
		{
			buffer.resize(this->HeaderRead_BlockSize());
			std::memcpy(buffer.data(), this->LookupObjectBlock(), this->ObjectBlockSizeInBytes());
		}

		void Object::CopyBlob(std::vector<Record>& buffer) const
		{
			buffer.resize(this->BlobSize());
			std::memcpy(buffer.data(), this->Blob_, this->BlobSizeInBytes());
		}

		bool Object::DeepCmp(const Object a, const Object b)
		{
		...
		std::unique_ptr<Object, Object::UniquePtrObjectDeleter> Object::AllocateUnique(const U32 sizeInRecords)
		{
			if (sizeInRecords == 0)
			[[unlikely]]
			{
		...
		template <>
		bool Object::DeepValueCmp_Equal<U64>(const Object a, const Object b)
		{
		...
		template <>
		bool Object::DeepValueCmp_Equal<I64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Equal<F64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Less<U64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Less<I64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Less<F64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_LessEqual<U64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_LessEqual<I64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_LessEqual<F64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Greater<U64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Greater<I64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_Greater<F64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_GreaterEqual<U64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_GreaterEqual<I64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		template <>
		bool Object::DeepValueCmp_GreaterEqual<F64>(const Object a, const Object b)
		{
			// If their size is not equal, their values cannot be equal too.
			if (a.HeaderRead_BlockSize() != b.HeaderRead_BlockSize())
			{
		...
		[[maybe_unused]]
		static VerboseReactorDescriptor CreateDescriptor
		(
			FixedStack& stack,
		...
		std::pair<ReactorShutdownReason, const ReactorState&> Reactor::Execute(ByteCode::AppCodeBundle&& bundle)
		{
		...
		std::pair<ReactorShutdownReason, ReactorState> SingletonExecutionProxy(
		...
		ReactorCoreSpecialization SmartSelectReactor(const System::CpuFeatureDetector& cpuFeatureDetector)
		{
#if NOX_ARCH_X86_64
...
#	error "ARM64 not yet supported!"
		...
		const ReactorRegistry& GetReactorRegistry()
		{
			return REACTOR_REGISTRY;
		}

		ReactorRoutineLink::ReactorRoutineLink(const ReactorCoreSpecialization specialization,
		...
		ReactorRoutineLink GetFallbackRoutineLink()
		{
		...
		ReactorCoreExecutionRoutine* GetReactorRoutineFromRegistryByTarget(const ReactorCoreSpecialization target)
		{
		...
		ReactorRoutineLink GetOptimalReactorRoutine(const System::CpuFeatureDetector& features)
		{
		...
		ReactorShutdownReason SingletonExecutionProxy(const VerboseReactorDescriptor& input, ReactorState& output,
		...
		const void** QueryJumpTable(ReactorCoreExecutionRoutine& routine)
		{
		...
		std::size_t ReactorPool::SmartQueryReactorCount(const std::size_t desired)
		{
		...
		void PrintShutdownReason(const ReactorShutdownReason reason, const InterruptAccumulator code)
		{
...
#	error "Unknown arch!"
		...
		void TaskQueueThread::DispatchJobQueue()
		{
			for (;;)
			{
				TaskRoutine routine;
		...
		void TaskQueueThread::Enqueue(TaskRoutine&& target)
		{
			std::lock_guard lock{this->QueueMutex_};
		...
		ReactorValidationResult VerboseReactorDescriptor::Validate() const
		{
