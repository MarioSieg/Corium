...
{
	Proxy::Proxy(ISubsystem* const instance, std::unique_ptr<SubsystemConfig>&& config, void* const userData) : Handle_ { instance }
	{
	...
	Proxy::Proxy(Proxy&& other) noexcept : Handle_ { std::move(other.Handle_) }
	{
		other.Handle_ = nullptr;
	}

	auto Proxy::operator =(Proxy&& other) noexcept -> Proxy&
	{
		if (this == &other)
		{
			return *this;
	...
	Proxy::~Proxy()
	{
		...
		if (this->Handle_->HasSubscribed(HookFlags::OnDestruct))
		{
			this->Handle_->Invoke<HookFlags::OnDestruct>();
		}
	}
}
