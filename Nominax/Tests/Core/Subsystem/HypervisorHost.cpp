...
#include "HostMockWithAllEvents.hpp"

TEST(HypervisorHost, Install)
{
    MockSubsystemWithAllEvents::MockCounter = 0;
    MockHypervisorHostWithAllEvents host { };
    const auto key { host.Install<MockSubsystemWithAllEvents>() };
    ASSERT_EQ(MockHypervisorHostWithAllEvents::MockCounter, 2);
...
{
    MockSubsystemWithAllEvents::MockCounter = 0;
    MockHypervisorHostWithAllEvents::MockCounter = 0;
    MockHypervisorHostWithAllEvents host { };
    const auto key { host.Install<MockSubsystemWithAllEvents>() };
    ASSERT_EQ(MockSubsystemWithAllEvents::MockCounter, 2);
    ASSERT_EQ(MockHypervisorHostWithAllEvents::MockCounter, 2);
    MockSubsystemWithAllEvents::MockCounter = 0;
    host.Uninstall(key);
    ASSERT_EQ(MockHypervisorHostWithAllEvents::MockCounter, 4);
    ASSERT_EQ(MockSubsystemWithAllEvents::MockCounter, 2);
...
TEST(HypervisorHost, PauseResume)
{
    ...
    host.Pause(key);
    ASSERT_EQ(MockHypervisorHostWithAllEvents::MockCounter, 1);
